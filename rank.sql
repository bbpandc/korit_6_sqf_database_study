...
from
	(select
		...
		-- publisher_id가 같은 값들 안에서 book_count가 큰 수 부터 번호를 매긴다(수가 같으면 공동 순위로 -> 1등, 2등, 2등, 4등). 
	#	rank() over(partition by publisher_id order by book_count desc) as `rank`,
    
		-- publisher_id가 같은 값들 안에서 book_count가 큰 수 부터 번호를 매긴다(수가 같으면 공동 순위로 -> 1등, 2등, 2등, 3등). 
	#	dense_rank() over(partition by publisher_id order by book_count desc) as `dense_rank`,
    
		-- publisher_id가 같은 값들 안에서 book_count가 큰 수 부터 번호를 매긴다(수가 같으면 행의 순서대로 -> 순위 중복x).
